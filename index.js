...
let physicsErrorLogged = false;

const GRAVITY = 0.08;
const DRAG = 0.98;
const FRICTION = 0.6;

function startPhysicsLoop(entityId) {
  stopPhysicsLoop();
...
function connectBot() {
  ...
  client.on('start_game', (packet) => {
    ...
    // نلتقط الموقع الأولي من نفس الحزمة (player_position: vec3f)
    if (packet.player_position) {
      botPosition = { x: packet.player_position.x, y: packet.player_position.y, z: packet.player_position.z };
    }
    if (packet.rotation) {
      botPitch = packet.rotation.x || 0;
      botYaw = packet.rotation.y || 0;
    }
    ...
    try {
      ...
      startPhysicsLoop(runtimeEntityId);
    } catch (e) {
  ...
  client.on('set_entity_motion', (packet) => {
    if (!botPosition || runtimeEntityId === null) return;
