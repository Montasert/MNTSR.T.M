...
// ============================================
// حالة المزاج الداخلية — تتغير حسب الأحداث وتؤثر
// على أسلوب رد البوت بالشات دون أي حركة فعلية
// ============================================
let annoyance = 0;        // 0 = هادئ، يرتفع مع الضرب المتكرر
let lastHitAt = 0;
let recentHits = [];      // طوابع زمنية لآخر ضربات، لتمييز جد من مزح
const ANNOYANCE_DECAY_MS = 60000; // يهدأ تدريجياً بعد دقيقة بدون ضرب

function currentMoodDescription() {
  const hour = new Date().getHours(); // وقت السيرفر المستضاف (تقريبي)
...
// ============================================
// ردود فورية جاهزة عند الضرب (بدون استدعاء API)
// توفّر الحصة المجانية، وتُستخدم للضربة الأولى الخفيفة
// ============================================
const LIGHT_HIT_REPLIES = [
  'ايش هالضربة؟ 😅',
  'هاها طيب طيب',
  'وقف بضربني 😂',
  'ليش هيك 😆',
...
const SERIOUS_HIT_REPLIES = [
  'وقف!! ليش عم تضربني!!',
  'خلص بجد؟؟ 😠',
  'طيب طيب هدّي، عم تعصبني',
  'استوقف قبل ما تندم',
];
...
function registerHit() {
  ...
  // تصفير تدريجي للانزعاج لو مرت فترة هدوء طويلة
  if (now - lastHitAt > ANNOYANCE_DECAY_MS) {
    annoyance = 0;
    recentHits = [];
  }
  lastHitAt = now;

  // نحتفظ بس بالضربات اللي صارت خلال آخر 5 ثواني لتمييز الهجوم الجدي
  recentHits = recentHits.filter(t => now - t < 5000);
  recentHits.push(now);

  const isSerious = recentHits.length >= 3; // 3 ضربات خلال 5 ثواني = جدي
  if (isSerious) annoyance = Math.min(annoyance + 2, 8);
  else annoyance = Math.min(annoyance + 1, 8);
...
// مبني على تعريف حزمة move_player الرسمي لنسخة 1.26.30:
// runtime_id و ridden_runtime_id = أرقام عادية (varint)
// tick = BigInt (varint64)
// ============================================
// ============================================
// ملاحظة: نظام الحركة الفعلية (Knockback حركي) تم التخلي عنه
// نهائياً بسبب مخاطر استقرار حقيقية. بدلاً منه نستخدم رد فعل
// نصي بالشات (registerHit + LIGHT/SERIOUS_HIT_REPLIES بالأعلى).
// ============================================
...
function connectBot() {
  ...
  client.on('start_game', (packet) => {
    ...
    try {
      ...
      // الحركة الفعلية معطّلة نهائياً — رد الفعل عند الضرب أصبح نصياً بالشات
      // (انظر معالج set_entity_motion بالأسفل).
    } catch (e) {
  ...
  client.on('set_entity_motion', (packet) => {
    ...
    // أول ضربة أو ضربة خفيفة: رد جاهز سريع بدون استهلاك حصة API
    // هجوم متكرر جدي: نستخدم Gemini حتى يرد برد مخصص أكثر حدة
